...
import { QWERTY_LAYOUT, KeyPosition } from './keyboardLayout';
...
export function generateHeatmapSVG(
  ...
  for (const keyPos of QWERTY_LAYOUT.keys) {
    ...
    if (showLabels) {
      ...
      if (frequency > 0) {
        svgElements.push(
          `<text x="${textX}" y="${textY - 2}" text-anchor="middle" dominant-baseline="middle" ` +
          `font-family="Arial, sans-serif" font-size="${fontSize}" font-weight="bold" fill="#333">${displayKey}</text>`
        );
        svgElements.push(
          `<text x="${textX}" y="${textY + fontSize}" text-anchor="middle" dominant-baseline="middle" ` +
